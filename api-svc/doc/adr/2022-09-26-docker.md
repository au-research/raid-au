...
**Status note (2026-04):** Docker is still in use as the deployment unit.
Container images are pushed to AWS ECR (see `buildSrc/src/main/groovy/raido/EcrUtil.groovy`).
The api-svc, IAM (Keycloak), and SSO (SATOSA) services each have their own
Dockerfile. The "one container per node / ASG" model described above remains
the current approach; a move to a managed container service (ECS, etc.) has
not yet occurred.
