...
public class DataciteAttributesDtoFactory {
    ...
    public DataciteAttributesDto create(RaidCreateRequest request, String handle) {
        ...
        final var event = request.getAccess().getType().getId() == AccessTypeIdEnum.HTTPS_VOCABULARIES_COAR_REPOSITORIES_ORG_ACCESS_RIGHTS_C_ABF2_ ?
                "publish" : "register";
        ...
        if (request.getOrganisation() != null) {
            contributors.addAll(request.getOrganisation().stream()
                            .filter(organisation -> !organisation.getRole().stream()
                                    .filter(r -> r.getId() != OrganizationRoleIdEnum.HTTPS_VOCABULARY_RAID_ORG_ORGANISATION_ROLE_SCHEMA_186)
                                    .toList()
            ...
            fundingReferences.addAll(request.getOrganisation().stream()
                    .filter(organisation -> organisation.getRole().stream()
                            .anyMatch(r -> r.getId() == OrganizationRoleIdEnum.HTTPS_VOCABULARY_RAID_ORG_ORGANISATION_ROLE_SCHEMA_186)
                    )
        ...
        final var primaryTitle = request.getTitle().stream()
                .filter(t -> t.getType().getId() == TitleTypeIdEnum.HTTPS_VOCABULARY_RAID_ORG_TITLE_TYPE_SCHEMA_5)
                .findFirst()
                .orElseThrow();
        ...
        if (request.getRelatedObject() != null) {
            relatedIdentifiers.addAll(request.getRelatedObject().stream()
                            .filter(relatedObject -> !(relatedObject.getCategory().stream()
                                    .anyMatch(c -> c.getId() == RelatedObjectCategoryIdEnum.HTTPS_VOCABULARY_RAID_ORG_RELATED_OBJECT_CATEGORY_ID_191)
                                    &&
                                    relatedObject.getType().getId() == RelatedObjectTypeIdEnum.HTTPS_VOCABULARY_RAID_ORG_RELATED_OBJECT_TYPE_SCHEMA_272)
                            )
